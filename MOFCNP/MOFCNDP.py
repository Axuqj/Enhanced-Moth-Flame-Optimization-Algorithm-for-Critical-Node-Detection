...
class PyMoF:
    ...
    def ini(self,SearchAgents_no,dim,ub,lb,pwp):
        ...
        if isinstance(lb,int):  #判断下界是同一个数字，还是一个数列；下界相同，均为同一个数
            ...
            # print(len(population[3]),population[3])
    ...
    def MFO(self,N,Max_iteration,lb,ub,dim,  t_max,pwp):
        ...
        while time.time()-t0 < t_max:
            ...
            #高斯变异和扰动两点互换
            newm=np.transpose(Moth_pos)
            stda = [ np.std(newm[i])    for i in range(dim)]  #标准差
            for i in range(N):
                mution= []
                for j in range(dim):
                    mutn=Moth_pos[i][j]*rd.gauss(0, stda[j] )
                     # 高斯变异
                    no= [ int(Moth_pos[i][q])  for q in range(dim) if q <j ]     
                    mutn=self.check(mutn,ub,lb,no)
                    mution.append(mutn)

                mu_obj= self.fobj(mution)
                nowf =self.fobj(Moth_pos[i])
                if mu_obj < nowf:     
                    Moth_pos[i]= mution            
                    nowf=  mu_obj  
                ...
                iten=5
                item=0
                pw = math.ceil(self.K*pwp) #扰动
                while item< iten:        
                    item+=1        
                    pcount=0
                    while pcount<pw:
                        # print(len(S),pw)
                        new_node=rd.choice(list(Moth_pos[i]))
                        Moth_pos[i].remove(new_node)
                        pcount+=1

                    pcount=0
                    while pcount<pw:
                        new_node=rd.choice(list(self.g.nodes-Moth_pos[i]))
                        Moth_pos[i].append(new_node)   
                        pcount+=1
                    
                    #两点互换  
                    for t in range(30) : 
                        newn1=rd.choice(list(self.g.nodes-Moth_pos[i])) #随机选一个点
                        newS= Moth_pos[i] + [newn1]
                        L = sorted(newS,  key=lambda x: self.fobj(set(newS)-{x} )) 
                        newS.remove(L[0])
                        newf=self.fobj(newS)
                        if newf <= nowf:
                            Moth_pos[i]= newS
